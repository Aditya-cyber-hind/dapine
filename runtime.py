...
class Runtime:
    ...
    def execute_filter(self, step):
        ...
        if errors and len(filtered) == 0:
            raise RuntimeError(f"Filter failed on all rows. First error: {errors[0]}", step.line)
        name = step.alias or f"_df_{len(self.dataframes)}"
                    ...
                    for r in rows:
                        ...
                        if val is not None:
                            try:
                                ...
                                pass
                    if agg.func == "sum":
                        out_row[agg.output_name] = sum(values)
                    elif agg.func == "avg":
                        out_row[agg.output_name] = sum(values) / len(values) if values else 0
                    elif agg.func == "min":
                        out_row[agg.output_name] = min(values) if values else None
                    elif agg.func == "max":
                        out_row[agg.output_name] = max(values) if values else None
        ...
        for i, row in enumerate(df.rows):
            ...
            try:
                ...
                raise RuntimeError(f"Mutate failed on row {i}: {e}", step.line,
                                 hint=f"Check expression for column '{step.new_column}'")
    ...
    def execute_cast(self, step):
        ...
        for row in df.rows:
            ...
            try:
                if step.new_type == "int":
                    row[step.column] = int(float(val)) if val is not None else None
                elif step.new_type == "float":
                    row[step.column] = float(val) if val is not None else None
                elif step.new_type == "string":
                    row[step.column] = str(val) if val is not None else ""
                elif step.new_type == "bool":
                    row[step.column] = bool(val)
                elif step.new_type == "date":
                    if isinstance(val, str):
                        row[step.column] = date.fromisoformat(val)
    ...
    def execute_stats(self, step):
        ...
        for col in df.schema:
            ...
            if values:
                if len(values) > 1:
                    print(f"  {col}: min={min(values)}, max={max(values)}, "
    ...
    def execute_case(self, step):
        ...
        for row in df.rows:
            ...
            for case in step.cases:
                case_val = self._eval_atomic(case.value, row)
                if val == case_val:
                    matched = True
                    for s in case.body:
                        self.variables["_current_row"] = row
                        self.execute_step(s)
                    break
            if not matched and step.default_alias:
                for s in step.default_alias:
                    self.variables["_current_row"] = row
                    self.execute_step(s)
        return df

    def execute_if(self, step):
        condition_result = self.eval_condition(step.condition, {})
        if condition_result:
            for s in step.if_body:
                self.execute_step(s)
        elif step.else_body:
            for s in step.else_body:
                self.execute_step(s)
        return None
    ...
    def _eval_atomic(self, expr, row):
        if isinstance(expr, BinaryOp):
            return self.eval_condition(expr, row)
        elif isinstance(expr, ArithOp):
            return self.eval_arithmetic(expr, row)
        elif isinstance(expr, FuncCall):
            return self._eval_func_call(expr, row)
            ...
            return expr.value
        elif isinstance(expr, DateLiteral):
            return expr.value
    ...
    def _eval_func_call(self, expr, row):
        ...
        elif expr.name == "concat":
            return "".join(str(a) for a in args)
        elif expr.name == "abs":
            return abs(args[0]) if args[0] is not None else None
        elif expr.name == "round":
            n = args[1] if len(args) > 1 else 0
            return round(args[0], n) if args[0] is not None else None
        elif expr.name == "ceil":
            return math.ceil(args[0]) if args[0] is not None else None
        elif expr.name == "floor":
            return math.floor(args[0]) if args[0] is not None else None
        elif expr.name == "sqrt":
            return math.sqrt(args[0]) if args[0] is not None and args[0] >= 0 else None
        elif expr.name == "pow":
            return math.pow(args[0], args[1]) if len(args) > 1 else None
        elif expr.name == "year":
            if isinstance(args[0], date):
                return args[0].year
            return None
        elif expr.name == "month":
            if isinstance(args[0], date):
                return args[0].month
            return None
        elif expr.name == "day":
    ...
    def eval_condition(self, expr, row):
        if isinstance(expr, BinaryOp):
            ...
            if expr.op == "contains":
                return str(left).find(str(right)) >= 0 if left and right else False
            if expr.op == "starts_with":
                return str(left).startswith(str(right)) if left and right else False
            if expr.op == "ends_with":
                return str(left).endswith(str(right)) if left and right else False
            if expr.op == "matches":
                import re
                try:
                    return bool(re.search(str(right), str(left))) if left and right else False
                except re.error:
                    return False
